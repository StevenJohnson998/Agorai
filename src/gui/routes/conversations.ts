...
export function createConversationRoutes(store: IStore, fileStore?: IFileStore, fileStoreConfig?: { maxFileSize: number; allowedTypes: string[] }) {
  ...
  router.post("/c/:id/add-participant", async (req, res) => {
    ...
    res.render("partials/participants", {
      ...
      subscribers,
      agents: agents.filter((a) => !rawSubscribers.some((s) => s.agentId === a.id) && (a.type === "human" || a.type === "claude-code" || a.type === "claude-desktop" || a.type === "internal" || a.type === "orchestrator" || a.capabilities.includes("chat"))),
      envPath,
      basePath: bp,
      layout: false,
    });
